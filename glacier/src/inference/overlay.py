...
def hex_to_bgr(hex_str: str) -> Tuple[int, int, int]:
    """Converts hex color string to OpenCV BGR tuple (for use on BGR arrays)."""
    h = hex_str.lstrip('#')
    rgb = tuple(int(h[i:i+2], 16) for i in (0, 2, 4))
    return (rgb[2], rgb[1], rgb[0])  # BGR for OpenCV on BGR arrays

def hex_to_rgb(hex_str: str) -> Tuple[int, int, int]:
    """Converts hex color string to (R, G, B) tuple for drawing on RGB arrays."""
    h = hex_str.lstrip('#')
...
def create_boundary_overlay(
    ...
    base_rgb is in RGB order — uses hex_to_rgb so colors are preserved correctly.
    """
    img = base_rgb.copy()
    ...
    rgb_color = hex_to_rgb(color_hex)   # (R, G, B) – correct for RGB array
    ...
    cv2.drawContours(img, contours, -1, rgb_color, line_thickness)
    return img
...
def create_dual_epoch_overlay(
    ...
    color_y1_hex: str = "#2563EB",  # Vivid Blue (T1)
    color_y2_hex: str = "#DC2626",  # Vivid Red  (T2)
    line_thickness: int = 2
) -> np.ndarray:
    """
    Renders multi-temporal boundary comparison (Panel b):
    Blue filled semi-transparent areas for Year 1, Red for Year 2,
    with crisp opaque contour outlines on top.
    base_rgb is in RGB order — uses hex_to_rgb for correct colour rendering.
    """
    img = base_rgb.copy()
    if img.dtype != np.uint8:
        img = np.clip(img * 255.0, 0, 255).astype(np.uint8)

    # ---- Year 1: Blue semi-transparent fill + crisp outline ----
    m1 = (mask_year1 > 0).astype(np.uint8)
    cnts1, _ = cv2.findContours(m1, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    blue_rgb = hex_to_rgb(color_y1_hex)   # (R, G, B) for RGB array
    if cnts1:
        fill1 = img.copy()
        cv2.drawContours(fill1, cnts1, -1, blue_rgb, -1)
        img = cv2.addWeighted(fill1, 0.30, img, 0.70, 0)
        cv2.drawContours(img, cnts1, -1, blue_rgb, max(line_thickness, 2))

    # ---- Year 2: Red semi-transparent fill + crisp outline ----
    m2 = (mask_year2 > 0).astype(np.uint8)
    cnts2, _ = cv2.findContours(m2, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    red_rgb = hex_to_rgb(color_y2_hex)    # (R, G, B) for RGB array
    if cnts2:
        fill2 = img.copy()
        cv2.drawContours(fill2, cnts2, -1, red_rgb, -1)
        img = cv2.addWeighted(fill2, 0.25, img, 0.75, 0)
        cv2.drawContours(img, cnts2, -1, red_rgb, max(line_thickness, 2))

    return img

# Canonical status → RGB lookup for Panel C dots (R, G, B order for RGB arrays)
_STATUS_RGB = {
    "Newly Formed": (241, 196, 15),   # Yellow  #F1C40F
    "Survived":     (46, 204, 113),   # Green   #2ECC71
    "Drained":      (231, 76, 60),    # Red     #E74C3C
}
...
def draw_classified_markers(
    ...
    Each marker: black outer shadow → white ring → vivid coloured fill.
    base_rgb is in RGB order — uses RGB tuples for correct colour rendering.
    """
    img = base_rgb.copy()
    if img.dtype != np.uint8:
        img = np.clip(img * 255.0, 0, 255).astype(np.uint8)
    ...
    for rec in summary_records:
        ...
        px = max(marker_radius + 1, min(int(px), w - marker_radius - 1))
        py = max(marker_radius + 1, min(int(py), h - marker_radius - 1))

        status = rec.get("status", "Survived")

        # Prefer canonical status-based RGB colour; fall back to hex field
        if status in _STATUS_RGB:
            rgb_color = _STATUS_RGB[status]
        else:
            color_hex = rec.get("color", config.COLOR_SURVIVED)
            rgb_color = hex_to_rgb(color_hex)

        # Layered circles: black outer → white ring → vivid colour fill
        cv2.circle(img, (px, py), marker_radius + 1, (0, 0, 0), -1)           # black shadow
        cv2.circle(img, (px, py), marker_radius,     (255, 255, 255), -1)     # white ring
        cv2.circle(img, (px, py), marker_radius - 2, rgb_color, -1)           # colour fill

    ...
    for geom_dict, val in extracted_shapes:
        if val == 1:
            ...
            props = {
                ...
                "perimeter_km": round(float(geom.length * (0.01 if is_pixel_space else 111.32)), 4),
                "centroid_lon": round(float(centroid.x), 6),
                "centroid_lat": round(float(centroid.y), 6),
                "pixel_x": int(round(centroid.x)),
                "pixel_y": int(round(centroid.y)),
            }
...
def mask_to_geodataframe(
    ...
    gdf_crs = None if crs == "PIXEL" else crs
    if len(poly_props) == 0:
    ...
    gdf = gpd.GeoDataFrame(records, geometry=geoms, crs=gdf_crs)
    return gdf

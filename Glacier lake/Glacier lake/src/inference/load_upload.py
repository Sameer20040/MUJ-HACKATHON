...
def load_input_image(
    ...
    if isinstance(file_source, (str, Path)):
        ...
        # Streamlit UploadedFile or BytesIO — always rewind to the start first
        # so repeated reads (e.g. across Streamlit reruns) get the full buffer.
        ...
        if hasattr(file_source, "getvalue"):
            # getvalue() returns the whole buffer regardless of pointer position
            bytes_data = file_source.getvalue()
        elif hasattr(file_source, "read"):
            bytes_data = file_source.read()
            # Rewind again so any later consumer can re-read the stream
            try:
                file_source.seek(0)
            except Exception:
                pass
        ...
        # Guard against an empty read (e.g. an already-consumed stream)
        if bytes_data is not None and len(bytes_data) == 0:
            raise ValueError(
                "Uploaded file stream was empty (0 bytes). The file pointer may have "
                "been consumed; re-upload the image or reset the stream to position 0."
            )

...
def run_scene_inference(
    ...
    override_transform: Optional[Any] = None,
    override_crs: str = "EPSG:4326",
) -> Dict[str, Any]:
    """
    ...
    transform = override_transform if override_transform is not None else (profile.get("transform") if profile else None)
    crs = override_crs if override_transform is not None else (str(profile.get("crs", "EPSG:4326")) if profile else "EPSG:4326")

...
def classify_temporal_change(
    ...
    for idx1, idx2, iou_score in matches:
        ...
        rec = {
            ...
            "pixel_x": int(round(row2.get("pixel_x", c.x))),
            "pixel_y": int(round(row2.get("pixel_y", c.y))),
        }
        records.append(rec)
    ...
    for idx1 in drained_indices:
        ...
        rec = {
            ...
            "centroid_lon": round(float(c.x), 6),
            "centroid_lat": round(float(c.y), 6),
            "pixel_x": int(round(row1.get("pixel_x", c.x))),
            "pixel_y": int(round(row1.get("pixel_y", c.y))),
        }
        records.append(rec)
        geometries.append(row1.geometry)

    crs = gdf_y2.crs if len(gdf_y2) > 0 else (gdf_y1.crs if len(gdf_y1) > 0 else None)
